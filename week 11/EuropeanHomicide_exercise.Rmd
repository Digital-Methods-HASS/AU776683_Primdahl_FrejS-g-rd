...
Western_Europe %>% 
  ...
  labs(title= "Homicide Rates in Western Europe Over Time",
       x = "Year",
       y = "Homicide Rate (per 100,000 peoples)",
...
```{r smooth graph-danish kings, eval = TRUE}
kings <- read_csv2( "data/Kongerækken-excel.csv",na="NA")

kings %>% 
  mutate(midyear=reg_slut-duration/2) %>% 
  ggplot(aes(x=midyear, y=duration))+
  geom_smooth(color= "darkolivegreen3")+
  labs(title="Duration of Danish Kings over Time",
       x = "Midyear",
       y = "Duration of Danish Kings",
       color = "Green")

#If we look at the Scandinavia graph from above, we can see that the homicide rates dropped over time. This coincide with our graph about Danish Kings as this graph shows that Danish Kings had a longer duration over time. This is, of course, because they did not get murdered since Homicide rates dropped.
```
...
ggsave("fig_output/Homicide Rates in Western Europe Over Time.png")
